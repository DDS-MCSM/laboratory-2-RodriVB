...
#                          Lab 2 - CPE Standard                                #
#                                                                              #
#              Rodrigo Vera - Data Driven Securty                     #
#                                                                              #
#******************************************************************************#

# Instalar paquete "xml2", si es necesario
...
Descargar <- function(){
  ...
  # Descargar archivo, si hace falta
...
GetCPEItems <- function(cpe_xml) {
  ...
  cpe_nombre <- xml2::xml_text(xml2::xml_find_all(cpe_xml, "//*[name()='cpe-item']/@name"))
  cpe_23 <- xml2::xml_text(xml2::xml_find_all(cpe_xml, "//*[cpe-23:cpe23-item]/*/@name"))
  cpe_titulo <- xml2::xml_text(xml2::xml_find_all(cpe_xml, "//*[cpe-23:cpe23-item]/*[@xml:lang='en-US'][1]"))
  cpes <- data.frame(cpe_nombre, cpe_titulo, cpe_23, stringsAsFactors = F)
...
CleanCPEs <- function(cpes){
  col_nom <- c("std", "std.v", "part", "vendor", "product",
                 "version", "update", "edition", "language", "sw_edition",
                 "target_sw", "target_hw", "other")
  
  cpes$cpe_23 <- stringr::str_replace_all(cpes$cpe_23, "\\\\:", ";")
  
  cpes <- tidyr::separate(data = cpes, col = cpe_23, into = col_nom, sep = ":", remove = F)
